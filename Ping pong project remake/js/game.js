...
addEvent(window,'keydown',handler);
addEvent(window,'keypress',handler);
addEvent(window,'keyup',handler);
...
function handler(event) {  
	...
	if (code == 97 || code == 1092) // руский и анлизуий вариант клавиш
 	first_player.moveUp();
 	if (code == 122 || code == 1103)
 	first_player.moveDown();
 	if (code == 107 || code == 1083)
 	second_player.moveUp();
 	if (code == 109 || code == 1100)
 	second_player.moveDown();
}
...
Game.start = function() { 
	...
	if (Menu.isLockStartButton == false) { // Защита от дурака
		Game.messegeClear(); // стереть сообщения если были
		Ball.move();
		Ball.show(); // лишним не будет
		/*Разблокировать обоих игроков*/
		first_player.locked = false; 
		second_player.locked = false;
	}
}
...
Game.stop = function() { 
	...
	Ball.stop(); 
	first_player.locked = true; // заблокировать площадки 
	second_player.locked = true;
}
...
Game.retry = function() {
	...
	Ball.show();
	/*Разблокировать площадки и кнопку старта*/
	first_player.locked = false; 
	second_player.locked = false;
	Menu.isLockStartButton = false;
}
...
function Field(goalMargin) {
	...
	Field.gameFieldHeight = gameField.clientHeight;  // высота поля
	Field.gameFieldWidth = gameField.clientWidth;
	Field.goalMarginLeft = goalMargin;
	Field.goalMarginRight = Field.gameFieldWidth - goalMargin; 
}

/*Сделал класс с прикидкой на будущее*/
function Menu() { // wrapper
}

Menu.isLockStartButton = false; // флаг блокировки кнопки старт
...
function Ball(speed) { 
	...
	Ball.moveInterval = null; // делаем переменные таймеров, нужно для того чтобы была возможность прервать выполение timeout или интервала
	Ball.startTimeout = null;
	Ball.vx = 0;  // смещение по координатным осям
	Ball.vy = 0;
	
	Ball.setInStartPosition = function() { /*Ставим по центру поля*/
		ball.style.left = Math.round(parseInt(Field.gameFieldWidth) / 2 - parseInt(ball.offsetWidth) / 2) + 'px';
		ball.style.top = Math.round(parseInt(Field.gameFieldHeight) / 2 - parseInt(ball.offsetHeight) / 2) + 'px';
	}
...
Ball.startDirection = function() {
	...
	//console.log(degr);
	} while ((degr > 0 && degr <= 70) || (degr >=140  && degr <= 270) || (degr >= 330 && degr < 360));
	
	var angle = degr * Math.PI / 180; // по уловию произвольный угол , но диапозон можно регулировать
	// нормальные градусные диапозоны это всё кроме (45deg, 135deg) & (225deg, 315deg)
	// счтиаем угол в радианах
	
	Ball.vx = Ball.speed * Math.sin(angle); // смещение по ОХ  , кстати sin cos в диапозоне [0,1]
	Ball.vy = -Ball.speed * Math.cos(angle); // здесь '-' потому что косинус - отножение гипотенузы (вестора скорости) к прилежажему катету получается верхний катет
}

 /*Указатель была ли опасная ситуация с застреванием мяча*/
...
Ball.move = function() {
	function fly() { 
		...
		if (isGoal()) { 
			...
			Ball.startDirection();
			Ball.setInStartPosition();
			Ball.startTimeout = setTimeout(function() { // та самая задержка в 2 секунды из условия
				Ball.move();
			}, 2000);
		}
...
/*Функция остановки мяча приходиться хитрить с интервалами чтобы можно было остановить игру когда угодно*/
Ball.stop = function() {
	if (Ball.moveInterval) { // чтобы остановить интервал
		clearInterval(Ball.moveInterval);
		Ball.moveInterval = null; /*Обязательно сбросить*/
	}
	if (Ball.startTimeout) { // аналогично для таймаута
		clearTimeout(Ball.startTimeout);
		Ball.startTimeout  = null;
	}
}
...
function Player(speed) { 
	...
	this.connectReflector = function(id) { /*Подключить площадки к игрокам*/
		...
		Player.prototype.moveDown = function() {
			if ( (this.reflector.getTop() + this.reflector.height + this.reflectorSpeed <= Field.gameFieldHeight) && (this.locked == false) ) // аналочино только для низа
				this.reflector.setTop(this.reflector.getTop() + this.reflectorSpeed);
		}
	}
}
...
setReflectorsInStartPosition = function() { // эта функция не внесена в класс так как устанавоивает в начальное положение обе площадки
	...
	var top = Math.round(parseInt(Field.gameFieldHeight) / 2 - parseInt(styles.height) / 2) + 'px';
	first.style.top = top;
...
function reflectionOX() {
	...
	return (
		(
			(ballLeft + Ball.speed <= 0) || (ballLeft >= (Field.gameFieldWidth - Ball.width)) // если вылезли за границы поля по OX сомнительно иначе был бы гол, но перестраховаться не помешает
		)
		|| 
		(
	...
	// return ( (ballLeft + ball.speed <= 0) || (ballLeft >= (Field.gameFieldWidth - ball.width)) ) ? true : false;
}
...
function reflectionOY() { // редкий случай но тоже надо учитывать если мяч попадёт на  меньшие стороны рокеток то надо оразить по OY
	...
	return (
		(
			(ballTop <= 0) || (ballTop >= (Field.gameFieldHeight - Ball.height) ) /* Общий случай */
		) 
		|| 
		(
